...
def main() -> None:
    ...
    print("=" * 60)
    fp16_path = Path("web/models/baloot-fan-v2.fp16.onnx")
    res_fp16 = evaluate_onnx_model(fp16_path, test_imgs, test_lbls, imgsz=704)
    print(json.dumps(res_fp16.to_dict(), indent=2))

    print("=" * 60)
    print("3. Evaluating INT8 ONNX model on holdout test set...")
    print("=" * 60)
    int8_path = Path("web/models/baloot-fan-v2.int8.onnx")
    res_int8 = evaluate_onnx_model(int8_path, test_imgs, test_lbls, imgsz=704)
